	...
	class MsgSocket
	{
		...
		private int WaitForData(int Size, int Timeout)
		{
			...
			try
			{
				...
				{
					Thread.Sleep(2);
					// My little change - check if TCPSocket is not null in case of frequently changes connections between PLC in my project
					SizeAvail = TCPSocket != null ? TCPSocket.Available : 0;
					//SizeAvail = TCPSocket.Available;
